...
class FairsenseGPURuntime(FairsenseRuntime):
    ...
    def predict_with_text_model(
        ...
        """
        Generates a response using the model on GPU and tracks emissions using the CodeCarbon library. Emissions data is saved to the directory specified by `emissions_output_dir`.
        ...
        """ 
        tracker = OfflineEmissionsTracker(country_iso_code="CAN", project_name="fairsense-gpu-inference", output_dir=self.emissions_output_dir)
        tracker.start()
        try:
            if progress:
...
class FairsenseCPURuntime(FairsenseRuntime):
    ...
    def predict_with_text_model(
        ...
        """
        Generates a response using Ollama for model inference and tracks emissions using the CodeCarbon library. Emissions data is saved to the directory specified by `emissions_output_dir`.
        ...
        str
            The text response generated by the model.
        """
        tracker = OfflineEmissionsTracker(country_iso_code="CAN", project_name="fairsense-cpu-inference", output_dir=self.emissions_output_dir)
        tracker.start()
        try:
            if progress:
